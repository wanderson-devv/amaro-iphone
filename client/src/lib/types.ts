...
export interface ServiceOrder extends Entity { number: number; customerUuid: string; customerName?: string; equipment: string; brand?: string; model?: string; serialNumber?: string; imei?: string; reportedIssue: string; diagnosis?: string; status: ServiceOrderStatus; technician?: string; expectedDelivery?: string; labor: number; discount: number; total: number; items: ServiceOrderItem[]; warrantyDays: number }
